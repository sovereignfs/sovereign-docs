...
| --- | --- | --- | --- |
| D-00 | **[PLUGIN]** Bootstrap this repo: `package.json`, `tsconfig` (extend `@sovereignfs/tsconfig`), ESLint/Prettier config matching `claude-sv` conventions, `manifest.json` skeleton (id `fs.sovereign.docs`, `type: sovereign`, `runtime: native`, `shell: default`), README pointing to SPEC.md + this roadmap. CI workflow deliberately deferred — see Deferred section. | — | ✅ |
...
- **D-00-ci** — A CI workflow for this repo. Drafted in D-00 (compose this
  checkout into a fresh `sovereignfs/sovereign` clone, then run format/lint/
  typecheck/test scoped to this plugin's files — necessary because it depends
  ...
  has one yet either — revisit once there's an established convention for
...
| Date | Change |
| --- | --- |
| 2026-07-17 | Completed D-04 drive config UI + server actions: `app/_lib/actions.ts` (`connectDrive`/`disconnectDrive`/`getDrive`, `useActionState`-compatible `ActionResult` convention) and `app/_components/ConnectDriveForm.tsx`/`DriveStatusCard.tsx`, wired into `app/page.tsx` as the plugin's current landing screen (projects/documents come with D-05/D-09; the drive-status card will likely move to a dedicated settings screen once that index exists). PAT-only, per SPEC.md — no OAuth code here, that's D-16. Stores the token via `sdk.secrets.create`, creates a `sdk.connections` record (`repoOwner`/`repoName`/`login` in `metadata`, matching SPEC's "display-only identity" split from `docs_drives`' `branch`/`base_path`), and publishes a `docs/.gitkeep` placeholder on first connect so the directory is visible on GitHub immediately. Disconnect only calls `sdk.connections.disconnect()` — no GitHub-side revoke call, since a PAT (unlike OAuth) has no programmatic revoke endpoint; SPEC's "calls the GitHub API to revoke first" line applies to D-16's OAuth flow, not this one. Extracted `parseRepository`/`sanitizeError` into `app/_lib/drive-rules.ts` (pure, unit-tested) since a `'use server'` file can only export async functions. Extended `detectGitHubRepository()` with an optional `token` param so the default branch can be auto-detected for private repos too, not just public ones. 7 new unit tests (23 total in the plugin). **Not verified live in the browser this pass** — a concurrent session sharing this same `claude-sv` checkout wiped this plugin's entire working tree (including `.git`) mid-task via what looks like a nested-repo-recursing `git clean`; recovered by re-cloning from `origin/main` (D-00–D-03 were already pushed) and redoing the uncommitted D-04 work, but skipped starting a second dev server on the same shared checkout to avoid another collision. |
| 2026-07-16 | Completed D-03 git provider adapter: `GitProvider` interface + `GitHubProvider` (`app/_lib/git-providers.ts`), ported from `sovereign-plainwrite`'s `_lib/git-providers.ts` per the deferred-shared-package plan. Takes a plain `GitRepoRef { owner, repo, branch }` rather than a project row — Docs has no per-project `repoOwner`/`repoName` columns like Plainwrite (those live in the `sdk.connections` record's `metadata` per SPEC.md), so the adapter stays decoupled from any `docs_*` table, which also keeps it closer to shared-package shape for D-16-shared. Added `listCommits()` (commits API filtered by path) beyond Plainwrite's original interface, since D-03's task text calls out "commits API for revisions" explicitly (used by D-08's revisions panel) and `getFileContent()` takes an optional `ref` so a revision can be read at a specific commit SHA, not just the branch head. 16 unit tests ported/extended from Plainwrite's suite, mocking `global.fetch`. |
| 2026-07-16 | Completed D-02 DB schema: `docs_drives`, `docs_projects`, `docs_documents`, `docs_drafts`, `docs_document_members` as Drizzle tables (`app/_db/schema.ts`, isolated SQLite store, `docs_` prefix per SPEC.md), generated SQLite + Postgres migrations (`migrations/sqlite`, `migrations/postgres`, `db/schema.postgres.ts` mirror per `docs/plugin-database.md`'s dual-schema convention). No `docs_credentials` table — the git token/connection lifecycle is platform-owned via `sdk.secrets`/`sdk.connections`, `docs_drives` only keeps `connection_id` + the fields read on every git call. Added `drizzle-orm`/`drizzle-kit` deps and `db:generate:sqlite`/`db:generate:pg` scripts, following `sovereign-shopper`'s pattern. |
| 2026-07-16 | Completed D-00 bootstrap: `package.json`, `tsconfig.json`, `manifest.json` (`shell: default` — `plugins/ledger` was earlier misread as using `minimal`; it actually uses `default` like every other product plugin), placeholder `EmptyState` page/layout matching Ledger's under-construction convention, README. A composed-clone CI workflow was drafted then dropped — see Deferred section. |
